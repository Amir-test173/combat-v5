...
class GameState extends ChangeNotifier {
  ...
  void toggleAlliance(String other){
    if(humanCountry==null||other==humanCountry||!countries.containsKey(other))return;
    if(multiplayer?.isConnected==true){
      if(areAllies(humanCountry!,other))multiplayer!.sendGameAction('endAlliance',target:other);
      else multiplayer!.sendGameAction('offerAlliance',target:other);
      return;
    }
    final otherCountry=countries[other]!;
    final key=_allianceKey(controllerRoot(me.controller),controllerRoot(otherCountry.controller));
    if(alliances.contains(key)){
      alliances.remove(key);
      _log('${tr('تم إنهاء التحالف مع','Alliance ended with')} ${countryDisplayName(otherCountry)}.');
    }else{
      alliances.add(key);
      _log('${tr('تم توقيع تحالف مع','Alliance signed with')} ${countryDisplayName(otherCountry)}.');
    }
    _sync();notifyListeners();
  }
  void respondAlliance(String offerId,bool accept){
    if(multiplayer?.isConnected==true){
      multiplayer!.sendGameAction('respondAlliance',value:'$offerId|${accept?'accept':'reject'}');
      return;
    }
    Map<String,dynamic>? offer;
    for(final item in diplomaticOffers){if(item['id']?.toString()==offerId){offer=item;break;}}
    if(offer==null||offer['status']!='pending')return;
    offer['status']=accept?'accepted':'rejected';
    if(accept){
      final from=offer['from']?.toString(),to=offer['to']?.toString();
      if(from!=null&&to!=null)alliances.add(_allianceKey(from,to));
    }
    _sync();notifyListeners();
  }
